...
module.exports = {
  ...
  crefProperty: function () {
    return _crefProperty;
  },
  crefOffice: function () {
    return _crefOffice;
  },
  crefNotification: function () {
    return _crefNotification;
  },
  // generate unique ids for collection rows
  genAgentId: function (callback) {
    return _getNextId(_pknAgentId, callback);
  },
  genClientId: function (callback) {
    return _getNextId(_pknClientId, callback);
  },
  genPropertyId: function (callback) {
    return _getNextId(_pknPropertyId, callback);
  },
  genOfficeId: function (callback) {
    return _getNextId(_pknOfficeId, callback);
  },
  genNotificationId: function (callback) {
    return _getNextId(_pknNotificationId, callback);
  },
  // create the Counter collection
  createCounterColl: function (callback) {
    return _createCounterColl(callback);
  },
...
function _dbInit(callback) {
  ...
  if (_dbConnectedInd == false) { // not yet connected, proceed and connect.
    // check if we have a local mongoDB service within the OpenShift project
    var mongoURL = _findMongoService();
    if (mongoURL) { // we found a local mongoDB within OpenShift, we will now use this DB 
      _mongoURL = mongoURL;
    }
    ...
    _mongoClient.connect(_mongoURL + '?maxPoolSize=8', connectOptions, function (err, database) {
      if (!err) { // connected!
        ...
        database.stats(function (err, stats) {
          ...
          var dbName = stats.db;
          if (dbName == 'admin') { // we are in the 'admin' DB!
            ...
            _dbref.addUser(_dbTargetUser, _dbTargetPswd, {
              roles: ['dbOwner']
            }, function (err, result) {
              if (err) {
      ...
      } else { // error occured while establishing connectivity to the DB
...
function _findMongoService() {
  ...
  if (host && port) { // we have located the mongoDB service host:port via env vars
...
function _getNextId(idType, callback) {
  ...
  _crefCounter.findAndModify({
      '_id': idType
    }, // find the matching '_id' for the given idType
    [
      ['_id', 'asc']
    ], // sort order
    {
    ...
    }, // auto increment the the sequence 'seq' counter within the given row
    {
      new: true,
      w: 1,
      j: 1
    }, // return the updated record/row after it's updated
    function (err, result) // callback
    { // callback for findAndModify() update
      if (!err && result.value) { // all is well, return the generated ID 
        ...
        callback(err, pkId);
      } else { // something went wrong, check that Counter collection is properly built! 
        ...
        _createCounterColl(
          function (err) {
            if (!err) { // Counter collection now exists
              // attempt to generate the ID again, retry one more time 
              _getNextId(idType, callback);
            } else { // ERROR! Counter collection does not exist or is corrupt and could not be rebuilt!
  ...
  _crefCounter.drop(
    function (err, reply) {
      ...
      counterRecords = [{
        ...
        _id: _pknNotificationId,
        seq: 4999
      }];
      // insert the records
      _crefCounter.insertMany(counterRecords, {
          w: 1,
          j: true
        },
        function (err, reply) {
